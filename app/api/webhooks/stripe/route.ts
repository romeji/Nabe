    ...
    await envoyerEmailFiable({
      ...
      html: `<p>${emailsContenu.vente_message || "Une nouvelle commande vient d'être payée :"}</p>
             <ul>
               ...
               <li><strong>E-mail :</strong> ${commande.clientEmail}</li>
               <li><strong>Téléphone :</strong> ${commande.clientTelephone || 'non renseigné'}</li>
               <li><strong>Adresse de livraison :</strong> ${commande.adresseLivraison || ''}, ${commande.codePostal || ''} ${commande.ville || ''}, ${commande.pays || ''}</li>
               <li><strong>Mode de paiement :</strong> ${commande.modePaiementLabel || 'non renseigné'}</li>
               <li><strong>Total :</strong> ${Number(commande.total).toFixed(2)} €</li>
               <li><strong>Articles :</strong><br/>${commande.lignes.map((l: any) => `&nbsp;&nbsp;• ${l.quantite} × ${l.nomProduit}${l.taille ? ` — taille ${l.taille}` : ''}`).join('<br/>')}</li>
             </ul>
