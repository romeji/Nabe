...
export default function ConnexionContenu() {
  ...
  }, [router, session, status, urlRetour]);
  ...
  async function gererConnexion(e: React.FormEvent) {
    ...
    try {
      ...
      if (!csrfRes.ok) {
        throw new Error('CSRF indisponible');
      }

      const { csrfToken } = await csrfRes.json();

      const reponse = await fetch('/api/auth/callback/credentials', {
        method: 'POST',
        credentials: 'same-origin',
        headers: {
      ...
      if (
        estCibleAdminSuspecte(data?.url) ||
        estCibleAdminSuspecte(reponse.url)
      ) {
        console.error(
          'Navigation vers /admin bloquée depuis la connexion client.',
        );
        setErreur(
          'Une erreur est survenue. Réessayez, ou contactez-nous si cela persiste.',
        );
        return;
      }
  ...
  async function gererConnexionGoogle() {
    ...
    try {
      ...
      const reponse = await fetch('/api/auth/signin/google', {
        method: 'POST',
        credentials: 'same-origin',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
      ...
      if (
        ...
        console.error(
          'Navigation vers /admin bloquée depuis la connexion Google.',
        );
        setErreur(
      ...
      <div className="connexion-carte">
        ...
        >
          ...
          >
            ...
            <path
              fill="#34A853"
              d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.9-2.26c-.8.55-1.84.86-3.06.86-2.36 0-4.36-1.59-5.08-3.73H.96v2.34C2.45 15.98 5.48 18 9 18z"
            />
            <path
              fill="#FBBC05"
              d="M3.92 10.69c-.18-.55-.29-1.13-.29-1.69s.11-1.14.29-1.69V4.97H.96A8.96 8.96 0 000 9c0 1.45.35 2.82.96 4.03l2.96-2.34z"
            />
            <path
              fill="#EA4335"
              d="M9 3.58c1.32 0 2.51.45 3.44 1.35l2.58-2.58C13.46.89 11.43 0 9 0 5.48 0 2.45 2.02.96 4.97l2.96 2.34C4.64 5.17 6.64 3.58 9 3.58z"
            />
          </svg>
